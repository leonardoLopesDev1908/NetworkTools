...
Proxy::Proxy(std::string* host, std::string* port, std::string* errorMessage)
...
std::expected<void, std::string> Proxy::start()
{
    ...
    while (proxyRun)
    {
        ...
        m_conns.enqueue([conn = newConn, this]() { conn->start(); });
    }
...
Queue<std::string>& Proxy::getErrors() { return m_errorQueue; }

bool Proxy::isRunning() const { return proxyRun; }

void Proxy::setKeep(bool keepFlag) { m_keepMessages = keepFlag; }
